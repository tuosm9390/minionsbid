...
import { Check, RefreshCw, Shield, Swords, X } from "@/components/ui/CyberIcons";
    ...
    >
      <div className="bg-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] w-full max-w-2xl max-h-[85vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="px-5 py-4 border-b-4 border-black flex items-center justify-between bg-minion-yellow">
          <div>
            <h3 className="flex items-center gap-2 text-lg font-black text-black">
              <ThreeDIcon name="cube" alt="테스트 데이터" size={28} />
              테스트 데이터 미리보기
            </h3>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-6 custom-scrollbar">
          <div>
            <p className="mb-2 flex items-center gap-2 text-sm font-black text-gray-700">
              <Shield size={16} />
              팀장 ({templateData.captains.length}명)
            </p>
          ...
          <div>
            <p className="mb-2 flex items-center gap-2 text-sm font-black text-gray-700">
              <Swords size={16} />
              경매 선수 ({templateData.players.length}명)
            </p>
            <div className="rounded-xl overflow-hidden border border-gray-100">
        ...
        <div className="px-6 py-4 border-t-4 border-black bg-white flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            <button onClick={onRegenerate} className="pixel-button flex items-center gap-2 bg-purple-100 text-purple-700 px-4 py-2.5 text-sm font-bold">
              <RefreshCw size={14} />
              다시 생성
            </button>
            <button onClick={onApply} className="pixel-button flex items-center gap-2 bg-black text-white px-8 py-3 text-sm font-heading">
              템플릿 적용
              <Check size={14} />
            </button>
          </div>
