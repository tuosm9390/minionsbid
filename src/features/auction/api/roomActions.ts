...
export async function saveAuctionArchive(
  ...
  try {
    ...
    const roomCollection =
      getAuctionServerServices().firestore.collection("rooms");
    const roomDocRef =
      typeof roomCollection.doc === "function"
        ? roomCollection.doc(payload.roomId)
        : null;
    ...
    if (roomDocRef && typeof roomDocRef.get === "function") {
      ...
      roomData =
        roomSnap &&
        typeof roomSnap === "object" &&
        "data" in roomSnap &&
        typeof roomSnap.data === "function"
          ? (roomSnap.data() ?? {})
          : {};
    }

    await getAuctionServerServices()
      .firestore.collection("auction_archives")
      .add({
