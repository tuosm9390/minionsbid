...
export async function drawNextPlayer(roomId: string): Promise<{ error?: string }> {
  try {
    ...
    await sysMsg(roomId, `🎲 ${picked.data().name} 선수 추첨!`)
    return {}
  } catch (err) {
...
export async function pauseAuction(roomId: string): Promise<{ error?: string }> {
  try {
    await adminDb.collection('rooms').doc(roomId).update({ timer_ends_at: null })
...
export async function resumeAuction(
  ...
  try {
    ...
    await sysMsg(roomId, '✅ 팀장이 재연결되었습니다. 5초 후 경매가 재개됩니다.')
    return { timerEndsAt: timerEndsAt.toISOString() }
  } catch (err) {
    const message = err instanceof Error ? err.message : '알 수 없는 오류'
    return { error: message }
...
export async function closeLotteryAction(
  ...
  try {
    await sysMsg(roomId, `🎰 ${playerName} 선수 추첨이 완료되었습니다!`)
...
export async function placeBid(
  ...
  if (!Number.isInteger(amount) || amount <= 0) return { error: '양의 정수 금액을 입력하세요.' }
  if (amount % 10 !== 0) return { error: '10P 단위로 입찰해야 합니다.' }
  if (amount > 100_000) return { error: '최대 입찰액(100,000P)을 초과했습니다.' }
  ...
  try {
    ...
    const timerField = roomData.timer_ends_at as admin.firestore.Timestamp | null
    if (!timerField) return { error: '경매가 진행 중이지 않습니다.' }
    if (timerField.toMillis() - Date.now() < -500) return { error: '경매 시간이 종료되었습니다.' }
    ...
    const currentTimer = currentTimerSnap.data()?.timer_ends_at as admin.firestore.Timestamp | null
    ...
    if (currentTimer) {
      ...
      if (currentRemaining < EXTEND_THRESHOLD_MS) {
        ...
        await sysMsg(roomId, `⏰ 타이머가 5초 연장! (${teamData.name} ${amount}P 입찰)`)
      } else {
        await sysMsg(roomId, `💰 ${teamData.name} 팀이 ${amount}P에 입찰했습니다!`)
      }
    }
...
export async function awardPlayer(
  ...
  try {
    ...
    const playerRef = adminDb.collection('rooms').doc(roomId).collection('players').doc(playerId)
    const roomRef = adminDb.collection('rooms').doc(roomId)
    let msgContent = ''
    ...
    await adminDb.runTransaction(async (tx) => {
      ...
      const roomData = roomSnap.data()
      const timerEndsAt = roomData?.timer_ends_at as admin.firestore.Timestamp | null
      ...
      if (topBid && teamRef && teamData) {
        ...
        msgContent = `🏆 ${teamData.name}이(가) ${playerData.name} 선수를 ${topBid.amount}P에 낙찰!`
      } else {
...
export async function draftPlayer(
  ...
  try {
    ...
    await sysMsg(roomId, `🤝 ${teamData.name}이(가) ${playerData.name} 선수를 자유계약으로 영입!`)
    return {}
  } catch (err) {
    const message = err instanceof Error ? err.message : '알 수 없는 오류'
    return { error: message }
...
export async function restartAuctionWithUnsold(
  ...
  try {
    ...
    await sysMsg(roomId, `🔄 유찰 선수 재경매를 시작합니다! (${unsoldSnap.size}명)`)
