...
import { POSITION_HEADER_KEYWORDS } from "../constants/room";
...
export function useCreateRoom() {
  ...
  const createRoom = async () => {
    ...
    const updated = [storedRoom, ...prev.filter((r) => r.id !== roomId)].slice(
      0,
      5,
    );
    ...
    setLinks({
      ...
      captainLinks: teamsResult.map((team) => ({
        ...
        link: team.invite_token
          ? `${baseUrl}/room/${roomId}?invite=${encodeURIComponent(team.invite_token)}`
          : `${baseUrl}/room/${roomId}?role=LEADER&teamId=${team.id}&token=${encodeURIComponent(team.leader_token)}`,
      })),
  ...
  const handleNext = async () => {
    if (step === 0) {
      ...
      if (!basic.teamCount || basic.teamCount < 2)
        return alert("팀은 최소 2개 이상이어야 합니다.");
      ...
      if (captains.some((c) => !c.name.trim() || !c.teamName.trim()))
        return alert("모든 팀장의 정보를 입력해주세요.");
      if (
        captains.some(
          (c) => c.captainPoints < 0 || c.captainPoints >= basic.totalPoints,
        )
      )
        return alert("포인트 설정을 확인해주세요.");
      syncPlayers(
        basic.teamCount *
          getAuctionSlotsPerTeam(basic.membersPerTeam, basic.captainMode),
      );
      ...
      if (players.find((p) => !p.name.trim()))
        return alert("모든 선수의 이름을 입력해주세요.");
      setIsLoading(true);
      try {
        await createRoom();
        setStep(3);
