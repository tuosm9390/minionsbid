...
로컬에서 현재 OS의 Chromium 대표 입찰 smoke를 실행합니다.

```bash
npm run test:e2e:auction:compat
```

실제 Ubuntu, Windows, macOS 동일성은 GitHub Actions의 `Auction realtime compatibility` 워크플로가 확인합니다. OS 매트릭스는 각 OS에서 Chromium 대표 경매 흐름을 실행하고, 브라우저 매트릭스는 Ubuntu에서 Chromium, Firefox, WebKit을 실행합니다.
...
기본 경매 흐름은 organizer와 모든 팀장이 모두 연결된 상태를 전제로 동작합니다. 연결이 하나라도 빠지면 organizer 화면이 즉시 경매를 일시정지하고, 모두 다시 연결되면 재개합니다.
...
- watchdog는 핵심 경매 상태를 자동 진행하지 않는다. organizer와 모든 팀장이 연결되지 않은 동안에는 입찰, 타이머, 낙찰 진행을 대신 밀어주지 않는다.

### Realtime debug markers

실시간 경매 품질 검증용으로 브라우저는 최근 입찰 marker를 `window.__auctionLatencyMarkers__`에 남길 수 있습니다.

- `client-response`: 입찰자가 direct bid 또는 서버 응답에서 받은 `eventId`
...
direct bid는 Firestore transaction의 bid `eventId`를 응답 marker와 후속 RTDB 이벤트에 함께 사용해 p95 중심 지연을 확인할 수 있게 합니다.
