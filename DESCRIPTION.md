...
- 변경 작업:
  ...
  - 입찰(placeBid)은 예외적으로 Firestore 클라이언트 SDK 직접 트랜잭션을 1차 경로로 사용하여 레이턴시를 최소화
  - 클라이언트 직접 입찰 성공 후에는 비동기 `broadcastBidEvent` Server Action이 RTDB 이벤트 전파, `last_auction_event` 저장, 시스템 메시지 생성을 뒤따라 처리

즉, 이 프로젝트는 클라이언트가 모든 상태를 직접 주도하는 구조가 아닙니다. 클라이언트는 실시간 상태를 렌더링하지만, 방 생성, 낙찰 처리, 경기 결과 등록, 아카이브 저장처럼 중요한 전환은 서버가 통제합니다. 입찰만이 레이턴시를 위해 클라이언트 직접 쓰기를 허용하며, 보안은 Firestore 규칙이 담당합니다.
